...
def _generic_runtime_release_block_message(artifact_type: str) -> str:
    ...
    if normalized == "cognition_revision":
        return (
            "cognition_revision cannot be marked runtime-eligible until it is canonicalized through "
            "the explicit cognition revision ratification path"
        )
...
def _cognition_revision_is_canonicalized(repo_root: Path, candidate_ref: str | None) -> bool:
    target_candidate_ref = str(candidate_ref or "").strip()
    if not target_candidate_ref:
        return False
    return cognition_revision_ratification_map(repo_root).get(target_candidate_ref) is not None
...
def can_release_runtime_authority(
    ...
    *,
    ...
    if not requires_runtime_ratification(normalized):
        return True
    if normalized == "cognition_revision":
        return _cognition_revision_is_canonicalized(repo_root, candidate_ref)
    if normalized == "profile_trait":
